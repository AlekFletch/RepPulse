...
|---|---|---|---|
| `@system.storage` (`get` / `set` / `delete` / `clear`) | Настройки (`AppSettings`), флаги | Key-value, callback-API. В OpenHarmony-документации указан лимит длины значения 128 байт (**TODO**: проверить для lite) → каждое поле настроек храним отдельным ключом | SDK |
