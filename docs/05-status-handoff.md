...
**2026-09-25, по видео с часов** ([08 §1.1](08-stage5.md)): калибровка и подсчёт срывались. Короткие толчки руки сбрасывали оценки силы тяжести и |g| посреди приседа, и глубина уплывала до −2,4 м. Теперь поворот руки определяется по устойчивому изменению направления, толчки его не вызывают, проверка «вбок» для приседаний убрана. Пакет `C:\Claude\RepPulse-haps\reppulse-stage5d.hap` (прошлые — в `old\`), 170 тестов.

**2026-09-25, второе видео:** сигнал медленно уходил на ±2 м, потому что руки понемногу опускаются за подход. Вертикальное ускорение теперь считается по модулю (от оценки направления не зависит), τ интегрирования 2 с. В «Диагностике» — усреднённый акселерометр в g с тремя знаками. Затем повтор закрывается, когда рука остановилась после подъёма (раньше при дрейфе ждал следующего приседа — «засчитывает с задержкой»). Пакет `C:\Claude\RepPulse-haps\reppulse-stage5f.hap`, 171 тест. Ждём снимки «Диагностики» в 6 положениях часов, чтобы оценить ошибки осей.
